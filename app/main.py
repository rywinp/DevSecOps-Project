...
# Will reject requests that don't match this request body
# with Status 422 Unprocessable Entity
# If we want strict strings, pydantic offers StrictStr type, because right
# now the client can send numbers for the content and filename fields
# and Pydantic will convert the number to a string, thus validating
# the request body
...
async def upload(item: ItemRequest) -> ItemResponse:
    ...
    # FastAPI will reject requests that
    # don't match the ItemRequest Pydantic Model
